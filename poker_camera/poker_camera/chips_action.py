...
class GreenDepthGrabber(Node):
    ...
    def __init__(self):
        ...
        PKG_DIR = Path(__file__).resolve().parent        # .../poker_camera
        CALIB_FILE = PKG_DIR / "T_gripper2camera.npy"    # 같은 폴더

        if not CALIB_FILE.exists():
            raise FileNotFoundError(f"Not found: {CALIB_FILE}")
        self.gripper2cam = np.load(CALIB_FILE)
    ...
    def execute_callback(self, goal_handle):
        ...
        if self._action_last_xyz is not None:
            ...
            x,y,z = self.transform_to_base((x,y,z))
    ...
    def transform_to_base(self, camera_coords):
        ...
        current = [242.915, 185.657, 256.981, 28.714, 179.979, 31.445]
