...
class PokerOrchestrator(Node):
    def __init__(self):
        ...
        self._handlers = {
            ...
            S.PICK_AND_SHOW:        self._st_pick_and_show,
            S.OPP_CARD_DETECT:      self._st_detect_and_show,
            S.OBSERVE_GESTURE:      self._st_observe_gesture,
    ...
    async def _st_hand_start(self):
        ...
        await self._exec_action(
            "Move(card)",
            self.ac_move,
            MoveNamedPose.Goal(name="card_grip", velocity=30.0, acc=30.0),
            retry=1
        )

        self.get_logger().info('Waiting: CARD_CHECK')
        await self._wait_intent(allowed={'CARD_CHECK'})
        return S.PICK_AND_SHOW

    async def _st_pick_and_show(self):
        """카드 집기 단계."""
        await self._exec_action(
            "PickCard",
            self.ac_pick,
            PickCard.Goal(approach_z=20.0, grip_width=600.0),
            retry=1
        )
    ...
    async def _st_detect_and_show(self):
        """
        SHOW + DETECT를 동시에 시작하고, 각 결과를 (ok/result/status)로 처리.
        - SHOW: ShowCard.Goal(timeout_s=3.0)
        - DETECT: DetectCardOnce.Goal(timeout_s=2.5)
        """
        # 동시에 시작
        ...
        detect_task = asyncio.create_task(
            self._exec_action("DetectCardOnce", self.ac_detect, DetectCardOnce.Goal(timeout_s=2.5), retry=0)
        )
    ...
    async def _st_observe_gesture(self):
        ...
        await self._exec_action(
            "Move(gesture_view)",
            self.ac_move,
            MoveNamedPose.Goal(name="gesture_view", velocity=30.0, acc=30.0),
            retry=1
        )

        # 제스처 관찰 (nose/arms/ear: bool로 해석)
        ...
        await self._exec_action(
            ...
            self.ac_move,
            MoveNamedPose.Goal(name="home", velocity=30.0, acc=30.0),
            retry=1
        )
    ...
    async def _agent_bet(self, act: str):
        ...
        if pay > 0:
            try:
                ok, res, status = await self._call_action(self.ac_bet, BetChips.Goal(amount=pay))
